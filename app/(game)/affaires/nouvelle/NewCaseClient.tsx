...
import { Shuffle, MapPin, Lock } from 'lucide-react'
import { LENGTH_MODES, isLengthAvailable, type LengthMode } from '@/lib/stripe'

const DIFFICULTIES = [
  { id: 'facile',    label: 'Facile',    desc: 'Indices clairs, 3-4 suspects. Idéal pour commencer.',       badge: 'badge-facile' },
  { id: 'moyen',     label: 'Moyen',     desc: 'Fausses pistes, mobile ambigu. Il faut recouper.',          badge: 'badge-moyen' },
  { id: 'difficile', label: 'Difficile', desc: 'Suspects convaincants, indices subtils. Pour fins limiers.', badge: 'badge-difficile' },
]
...
export default function NewCaseClient({ userPlan }: { userPlan: string }) {
  const router = useRouter()
  const [difficulty, setDifficulty] = useState('moyen')
  const [setting, setSetting] = useState('Paris, France')
  const [length, setLength] = useState<LengthMode>('moyenne')
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/generate-case', {
        ...
        body: JSON.stringify({ difficulty, setting, length }),
      })
      ...
      clearInterval(stepInterval)
      setError(err instanceof Error ? err.message : 'Impossible de générer l\'affaire.')
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-8 px-4">
        ...
        <div className="text-center">
          ...
          <p className="text-noir-mist font-typewriter text-sm italic min-h-[1.5em]">
        ...
        <p className="text-noir-smoke text-xs font-typewriter">L&apos;IA génère une affaire unique rien que pour vous...</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <div className="text-center mb-10">
        ...
        <h1 className="font-noir text-4xl font-black text-noir-cream mb-3">Nouvelle Enquête</h1>
        <p className="text-noir-mist italic text-sm">
          Choisissez votre difficulté, le cadre et la durée.<br />
      ...
      <div className="mb-8">
        ...
        <div className="grid gap-3">
          {DIFFICULTIES.map((d) => (
            ...
            >
              <span className={`px-2 py-0.5 rounded text-xs font-typewriter flex-shrink-0 ${d.badge}`}>{d.label}</span>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {(Object.entries(LENGTH_MODES) as [LengthMode, typeof LENGTH_MODES[LengthMode]][]).map(([id, mode]) => {
            const available = isLengthAvailable(id, userPlan)
            const isSelected = length === id
            return (
              <button
                key={id}
                onClick={() => available && setLength(id)}
                disabled={!available}
                className={`flex flex-col items-center gap-1.5 p-3 rounded border text-center transition-all ${
                  !available
                    ? 'border-noir-smoke/30 bg-noir-charcoal/20 opacity-50 cursor-not-allowed'
                    : isSelected
                    ? 'border-noir-gold bg-noir-sepia/20'
                    : 'border-noir-smoke hover:border-noir-mist bg-noir-charcoal/30'
                }`}
                title={!available ? `Disponible dès le grade ${PLAN_LABELS[mode.minPlan]}` : undefined}
              >
                <span className="text-xl">{mode.icon}</span>
                <span className={`text-xs font-typewriter font-bold ${isSelected ? 'text-noir-gold' : 'text-noir-silver'}`}>
                  {mode.label}
                </span>
                <span className="text-noir-smoke text-xs font-typewriter">{mode.messages} msg</span>
                {!available && (
                  <span className="flex items-center gap-0.5 text-noir-smoke/60 text-xs font-typewriter">
                    <Lock size={9} />
                    {PLAN_LABELS[mode.minPlan]}
                  </span>
                )}
              </button>
            )
      ...
      {error === 'daily_limit_reached' ? (
        <div className="mb-4 p-4 bg-amber-950/30 border border-amber-900/50 rounded">
          <p className="text-amber-300 text-sm font-typewriter mb-2">⚠ Limite quotidienne atteinte.</p>
          <p className="text-amber-400/70 text-xs font-typewriter mb-3">Revenez demain ou passez à un grade supérieur.</p>
          <a href="/tarifs" className="inline-block px-4 py-2 rounded border border-noir-gold/60 text-noir-gold text-xs font-typewriter hover:bg-noir-sepia/20 transition-colors">
            Voir les grades →
          </a>
        </div>
      ) : error ? (
